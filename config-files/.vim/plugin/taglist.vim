...
function! s:Tlist_Get_Buffer_Filetype(bnum)
    ...
    if bufloaded(a:bnum)
        ...
        " GOKBERK: active or hidden buffers
        return buf_ft
    else
        " GOKBERK: ignore inactive buffers
        return ''
    endif
...
function! s:Tlist_Window_Init()
    ...
    " gokberk: like <CR> but does not close the taglist window
    nnoremap <buffer> <silent> <Space>
                \ :call <SID>Tlist_Window_Jump_To_Tag('useopen', 0)<CR>
    inoremap <buffer> <silent> <Space>
                \ <C-o>:call <SID>Tlist_Window_Jump_To_Tag('useopen',0)<CR>
...
function! s:Tlist_Window_Refresh_File(filename, ftype)
    ...
    while i <= ttype_cnt
        ...
        if ttype_txt != ''
            let txt = '  ' . s:tlist_{a:ftype}_{i}_fullname " gokberk: avoid too many spaces
            if g:Tlist_Compact_Format == 0
...
function! s:Tlist_Parse_Tagline(tag_line)
    ...
    if g:Tlist_Display_Prototype
        let ttxt = ' ' . s:Tlist_Get_Tag_Prototype(s:fidx, s:tidx) " gokberk: avoid too many spaces in front
    else
        let ttxt = ' ' . tag_name " gokberk: avoid too many spaces in front
...
function! s:Tlist_Process_File(filename, ftype)
    ...
    if v:version > 601
        ...
        while cmd_output != ''
            ...
            " Extract the tag scope/prototype
            if g:Tlist_Display_Prototype
                let ttxt = ' ' . s:Tlist_Get_Tag_Prototype(fidx, tidx) " gokberk: avoid too many spaces in front
            else
                let ttxt = ' ' . tag_name " gokberk: avoid too many spaces in front
...
function! s:Tlist_Window_Jump_To_Tag(win_ctrl, ...)
    " gokberk
    if a:0 > 0
        let closetagwindow = a:1
    else
        let closetagwindow = 1
    end
    ...
    " gokberk: close whenever a tag is chosen
    if closetagwindow == 1
...
function! s:Tlist_Window_Show_Info()
    ...
    " Get the tag search pattern and display it
    " gokberk: THIS IS WHERE THE FUNCTION/TAG NAME IS DISPLAYED BELOW THE SCREEN
    " IF THE TAG IS LONG, THIS IS ANNOYING, SO I DISABLE IT.
